...
namespace Avalonia.Aurora.Wayland
{
    internal sealed class WlDataObject : IDataObject, IDisposable, WlDataOffer.IEvents
    {
        ...
        {
            if (MimeTypes.Contains(Avalonia.Aurora.Wayland.MimeTypes.Text) || MimeTypes.Contains(Avalonia.Aurora.Wayland.MimeTypes.TextUtf8))
                yield return DataFormats.Text;
            if (MimeTypes.Contains(Avalonia.Aurora.Wayland.MimeTypes.UriList))
                yield return DataFormats.Files;
        }

        public bool Contains(string dataFormat) => dataFormat switch
            {
                nameof(DataFormats.Text) => MimeTypes.Contains(Avalonia.Aurora.Wayland.MimeTypes.Text) || MimeTypes.Contains(Avalonia.Aurora.Wayland.MimeTypes.TextUtf8),
                nameof(DataFormats.Files) => MimeTypes.Contains(Avalonia.Aurora.Wayland.MimeTypes.UriList),
                _ => false
            };

        public string? GetText()
        {
            var mimeType = MimeTypes.FirstOrDefault(static x => x is Avalonia.Aurora.Wayland.MimeTypes.Text) ?? MimeTypes.FirstOrDefault(static x => x is Avalonia.Aurora.Wayland.MimeTypes.TextUtf8);
            if (mimeType is null)
                return null;
        ...
        public IEnumerable<IStorageItem>? GetFileNames()
        {
            ...
            var fd = Receive(Avalonia.Aurora.Wayland.MimeTypes.UriList);
