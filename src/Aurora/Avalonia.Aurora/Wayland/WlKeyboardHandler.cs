...
internal class WlKeyboardHandler : IDisposable
{
    ...
    public WlKeyboardHandler(AvaloniaAuroraWaylandPlatform platform, WlInputDevice wlInputDevice)
    {
        _platform = platform;
        _wlInputDevice = wlInputDevice;
        _wlKeyboard = platform.WlSeat.GetKeyboard();
        _wlKeyboard.Key += OnKey;
        _wlKeyboard.Keymap += OnKeymap;
        _wlKeyboard.Modifiers += OnModifiers;
        _wlKeyboard.Enter += OnEnter;
        _wlKeyboard.Leave += OnLeave;
        _wlKeyboard.RepeatInfo += OnRepeatInfo;
    ...
    private void OnKey(object? sender, WlKeyboard.KeyEventArgs e)
    {
        ...
        var state = (WlKeyboardKeyState)e.State;
        var eventType = state == WlKeyboardKeyState.Pressed ? RawKeyEventType.KeyDown : RawKeyEventType.KeyUp;
        var keyEventArgs = new RawKeyEventArgs(_keyboardDevice, e.Time, _window.InputRoot, eventType, avaloniaKey, _wlInputDevice.RawInputModifiers);
        ...
        if (state == WlKeyboardKeyState.Pressed)
        {
            ...
            if (text is not null)
            {
                var textEventArgs = new RawTextInputEventArgs(_keyboardDevice, e.Time, _window.InputRoot, text);
    ...
    private void OnKeymap(object? sender, WlKeyboard.KeymapEventArgs e)
    {
        var map = LibC.mmap(IntPtr.Zero, (int)e.Size, MemoryProtection.PROT_READ, SharingType.MAP_PRIVATE, e.Fd, 0);
        if (map == new IntPtr(-1))
        {
            LibC.close(e.Fd);
            return;
        ...
        var keymap = LibXkbCommon.xkb_keymap_new_from_string(_xkbContext, map, e.Format, 0);
        LibC.munmap(map, (int)e.Size);
        LibC.close(e.Fd);
    ...
    private void OnModifiers(object? sender, WlKeyboard.ModifiersEventArgs e)
    {
        _wlInputDevice.Serial = e.Serial;
        LibXkbCommon.xkb_state_update_mask(_xkbState, e.ModsDepressed, e.ModsLatched, e.ModsLocked, 0, 0, e.Group);
    ...
    private void OnEnter(object? sender, WlKeyboard.EnterEventArgs e)
    {
        _window = _platform.WlScreens.WindowFromSurface(e.Surface);
        _wlInputDevice.Serial = e.Serial;
        KeyboardEnterSerial = e.Serial;
    }

    private void OnLeave(object? sender, WlKeyboard.LeaveEventArgs e)
    {
        _wlInputDevice.Serial = e.Serial;
    ...
    private void OnRepeatInfo(object? sender, WlKeyboard.RepeatInfoEventArgs e)
    {
        _repeatDelay = TimeSpan.FromMilliseconds(e.Delay);
        _repeatInterval = TimeSpan.FromSeconds(1d / e.Rate);
    }
