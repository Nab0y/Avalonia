...
namespace Avalonia.Aurora.Wayland
{
    internal class WlRegistryHandler : WlRegistry.IEvents, IDisposable
    {
        ...
        {
            Console.WriteLine($"WlRegistry OnGlobal {name}:{@interface}:{version}");
        ...
        public void OnGlobalRemove(WlRegistry eventSender, uint name)
        {
            ...
            {
                Console.WriteLine($"WlRegistry OnGlobalRemove unknown name {name}");
                return;
            }

            Console.WriteLine($"WlRegistry OnGlobalRemove {glob.Name}:{glob.Interface}:{glob.Version}");
